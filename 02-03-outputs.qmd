...
## Tables
  ...
  - Use `readr::write_csv()` to save `.csv.gz` files that will be sent to output checkers.

    ``` r
    readr::write_csv(object, 
                     path = file.path("output", "mydata.csv.gz"))
    ```  
    (Note that in more recent versions of the **readr** package the `path` argument has been renamed to `file`.)
...
- Ensure your outputs are in the correct formats 
  - `.csv.gz` (not `.rds` or `.Rdata`) for tables of data
